...
public class ObjectSpawner : MonoBehaviour
{
    ...
    private void ChangePreview()
    {
        if (m_prefabsToSpawn.Length > 0) //if the array isnt empty
        {
            if (!m_prefabsToSpawn[m_prefabSelectIndex].Equals(null)) //if the selected entry isnt a null reference then instance the object
            {
                ...
                // For URP/Lit shader, we need to handle the material setup correctly
                Material[] previewMaterials = m_prefabsToSpawn[m_prefabSelectIndex].GetComponent<MeshRenderer>().sharedMaterials;
                foreach (var mat in previewMaterials)
                {
                    if (mat.shader.name == "Universal Render Pipeline/Lit")
                    {
                        // Handle URP-specific properties
                        mat.SetFloat("_Metallic", 0.5f); // Default metallic value
                        mat.SetFloat("_Smoothness", 0.5f); // Default smoothness value
                        mat.SetColor("_BaseColor", Color.white); // Default base color
                        mat.SetColor("_EmissionColor", Color.black); // Default emissive color
                    }
                }
            }
    ...
    public void SpawnPrefab(GameObject obj)
    {
        ...
        // Adjust the material for URP/Lit shader when the prefab is spawned
        MeshRenderer renderer = objInst.GetComponent<MeshRenderer>();
        if (renderer != null)
        {
